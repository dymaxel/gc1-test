...
class TDZebraBarcodeConfiguration(models.Model):
    ...
    currency_position = fields.Selection([
         ...
         default='after')
    column_type = fields.Selection([('1', '1'), ('2', '2'), ('3', '3')], string="No of Column", default='1', required=True)
    ...
    barcode_font_size = fields.Integer('Barcode Font Size')
    ...
    tax_round = fields.Boolean('Tax Round')
    is_company = fields.Boolean('Company')
    liquidation = fields.Boolean('Expriry Date')
    batch_no = fields.Boolean('Batch No')
    ...
    company_size = fields.Char('Company FontSize', default=7)
    liquidation_size = fields.Char('Expriry FontSize', default=7)
    batch_size = fields.Char('Batch FontSize', default=7)

    def apply(self):
        format_id = self.env.ref('td_zebra_barcode_labels.paperformat_tdzebrabarcode_label')
        page_setting = {'1': 26, '2': 51, '3': 76}
        if format_id:
            format_id.write({
                'page_width': page_setting.get(self.column_type),
