...
class FieldSecurity(models.Model):
    ...
    model_id = fields.Many2one('ir.model', index=True)
    field_id = fields.Many2one('ir.model.fields', index=True)
    ...
    def default_get(self, fields):
        vals = super(FieldSecurity, self).default_get(fields)
        vals['model_id'] = self.env.context.get('default_model_id')
        return vals
