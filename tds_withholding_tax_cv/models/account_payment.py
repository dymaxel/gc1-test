    ...
    def compute_tds_amnt(self):
        for payment in self:
            ...
            if payment.tds_type in ('including', 'excluding') and payment.tds_tax_id and payment.amount:
                applicable = True
                if payment.partner_id and payment.partner_id.tds_threshold_check:
                    applicable = payment.check_turnover(self.partner_id.id, self.tds_tax_id.payment_excess, payment.amount)
                ...
                # jawaid 18/11/2021
                # if payment.tds_type in ('including',
                #                         'excluding') and payment.tds_tax_id and payment.amount:
    ...
    def _prepare_payment_moves(self):
        ...
        for payment in self:
            ...
            sales_tax_repartition_lines = payment.sales_tds_tax_id.invoice_repartition_line_ids.filtered(
                lambda x: x.repartition_type == 'tax')
            if ((payment.tds_type == 'including' and payment.tds_tax_id and payment.tds_amt) or (
                    payment.sales_tds_type == 'including' and payment.sales_tds_tax_id and payment.sales_tds_amt)) and applicable and payment.bill_type != 'non_bill':
